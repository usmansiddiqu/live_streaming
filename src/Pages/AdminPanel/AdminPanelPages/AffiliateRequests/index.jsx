...
import getAdminRequest from "../../../../api/getAdminRequests";

function AdminAffiliateRequests() {
  const navigate = useNavigate();
  const [activeTab, setActiveTab] = useState("pending");
  const [data, setData] = useState([]);
  const handleButtonClick = (transactionId, userId) => {
    navigate(`/admin/edit_user_invoice/${transactionId}/${userId}`);
  };
                ...
                .map((transaction) => (
                  <tr>
                    ...
                    <td
                      ...
                      style={{ border: "1px solid #313133" }}
                    >
                      <div className="flex">{transaction?.userId?.name}</div>
                    </td>
                    <td
                      class="px-6 py-4 text-white "
                      style={{ border: "1px solid #313133" }}
                    >
                      <div className="flex">${transaction?.totalAmount}</div>
                    </td>
                    <td
                      class="px-6 py-4 text-white"
                      style={{ border: "1px solid #313133" }}
